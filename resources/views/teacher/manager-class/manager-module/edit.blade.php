...
@section('title')
    <a href="{{route('teacher.manager-class.index',[$title,$course])}}">Manajemen Kelas - {{$title}} - Mapel - {{$course}}</a> -  Ubah Module {{$module->title}}
@endsection
...
@section('content')
    ...
    <form action="{{route('teacher.manager-class.update-module',[$title,$course,$module->id])}}" method="post" role="form" enctype="multipart/form-data">
        @csrf
        {{method_field('PUT')}}
